...
public class ModRiftThemeRecipeProvider extends RiftThemeRecipeProvider {
    ...
    public void generate(HolderLookup.Provider registries, Consumer<ThemeRecipe> writer) {
        writer.accept(new ThemeRecipe.Builder(DeferredHolder.create(ModRiftThemes.RIFT_THEME_KEY, WanderersOfTheRift.id("cave")))
                .setPriority(-1)
                .build());
        writer.accept(new ThemeRecipe.Builder(DeferredHolder.create(ModRiftThemes.RIFT_THEME_KEY, WanderersOfTheRift.id("forest")))
